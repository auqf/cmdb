...
from .core import ObjectPermissionChecker
...
class BaseObjectPermissionManager(models.Manager):
    ...
    def assign_perm(self, perm, user_or_group, table, obj_id):
        """
        Assigns permission with given ``perm`` for an instance ``obj`` and
        ``user``.
        """
        ct = get_content_type(table)
        if not isinstance(perm, Permission):
            permission = Permission.objects.get(content_type=ct, codename=perm)
        else:
            permission = perm
        ...
        obj_perm, _ = self.model.objects.get_or_create(**kwargs)
        return obj_perm
    ...
    def remove_perm(self, perm, user_or_group, table, obj_id):
        """
        ...
        if self.is_generic():
            filters &= Q(object_pk=obj_id)
        else:
            filters &= Q(content_object__pk=obj_id)
