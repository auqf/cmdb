...
class FieldListSerializer(DeletedFilterListSerializer):

    def to_representation(self, data):
        data = data.order_by('order', '-id')
        return super().to_representation(data)
    ...
    def validate_name(self, value):
        if not re.match('[a-z][a-z-0-9]*$', value):
            raise serializers.ValidationError(
                "Name must be lowercase letters, numbers, hyphens the "
                "composition, And can only begin with a letter")
        return value
...
class ReorderFieldsSerializer(serializers.ModelSerializer):
    """
    用于排序字段的Serializer
    """
    fields = serializers.PrimaryKeyRelatedField(
        queryset=models.Field.objects.all(), write_only=True, many=True,
        label='字段', help_text='此动态表下的全部字段id的列表，如：[1, 2, 3]')
...
class TableSerializer(serializers.ModelSerializer):
    ...
    creator_username = serializers.CharField(
        read_only=True, source="creator.username")

    fields = FieldSerializer(
        required=False, many=True, help_text='字段', read_only=True)

    def validate_name(self, value):
        if not re.match('[a-z][a-z-0-9]*$', value):
            raise serializers.ValidationError(
                "Name must be lowercase letters, numbers, hyphens the "
                "composition, And can only begin with a latter"
            )
        return value
